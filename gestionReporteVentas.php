...
<div id="centralPanel">
	<form>
		<fieldset class="formOpcionesReportes">
			...
			<div id="campoCliente" class="parrafoCerrar oculto">
			<label for="cliente">Nombres: </label>
				<select class="campoMedioNormal" name="cliente" id="cliente">
					<option value="-1">- - -Selecciona un cliente- - -</option>
				<?php
				if ($rsC->num_rows) {
					while($rowC = $rsC->fetch_assoc()) {
						$idCliente = $rowC['idCliente'];
						$nomCliente = $rowC['nombreCliente'];
						$appCliente = $rowC['apellidosCliente'];
				?>
					<option value="<?php echo $idCliente; ?>"><?php echo $nomCliente ." ".$appCliente; ?></option>
				<?php
					}
				}	
				?>
			...
			<input type="radio" id="opcionBuscar1" name="opcionBuscar" checked="checked" value="1" />Buscar por fecha
			<input type="radio" id="opcionBuscar2" name="opcionBuscar" value="2" />Buscar por Cliente
			<hr />
			<div id="buscarPorFechas" class="parrafoCerrar">
			...
			<div class="parrafoCerrar oculto" id="intervaloFechas">
